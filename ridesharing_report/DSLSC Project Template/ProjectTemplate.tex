...
\newcommand{\datasettitle}[1]{\uline{#1}\penalty0\hspace{0pt}}
...
\subsection{Introduction}
    \paragraph{Smart cities and mobility policy} A \textbf{smart city} is an urban system that uses data, digital infrastructure, integrated governance and technology in general to improve public services, quality of life, and environmental sustainability, and in its huge city planning, the \textbf{mobility policy} is one of the core domains.
    \textbf{Urban transport choices shape access to jobs, healthcare, education, and social life}. For this reason, smart mobility policy combines \textit{public transport data, private mobility platforms, and spatial indicators} to evaluate efficiency, but also \textbf{equity across neighborhoods}.
    Smart-city mobility policy increasingly treats \textbf{digital platforms as part of urban transport infrastructure} alongside buses, rail, and cycling networks. Due to their modern way of functioning, with features like \textit{real-time matching, cashless payment, and citywide coverage}, they make \textbf{Transportation Network Providers (TNP)} such as Uber and Lyft attractive to planners seeking \textbf{flexible capacity}.
    And yet, their private, fare-based model differs fundamentally from public transit: \textbf{access depends on willingness and ability to pay}. Equity analysis must therefore ask who bears the cost relative to local income and, even more importantly, \textbf{how usage relates to existing public supply}.

    \paragraph{Choosing Chicago} Chicago is a revealing case. The city is the third largest in the United States, organised around a \textit{radial street grid} and an \textit{elevated rail loop} that anchors the \textbf{central business district (CBD)}, also called, in fact, The Loop. It also qualifies as a smart-city context: Chicago publishes open mobility and socioeconomic data through the Chicago Data Portal, integrates CTA operations with digital GTFS feeds, and supports neighbourhood-scale, evidence-based planning. Under the OECD definition of cities and functional urban areas \cite{oecd_fua2012}, Chicago is classified as a FUA in every aspect, e.g. the city core plus the surrounding commuting zone where most daily work and service trips are economically integrated. 
    
    \paragraph{Chicago Transportation and its issues} The CTA provides extensive bus and ``L'' rail service, but coverage and frequency thin toward the urban fringe. \textbf{Decades of residential segregation} have concentrated lower-income households on the \textbf{South and West Sides} while \textbf{employment, retail, and nightlife} remain strongly oriented toward the \textbf{Loop and lakefront.} Commuting patterns documented in spatial mismatch research \cite{kwan1998} show that \textbf{many residents travel long distances to central job clusters}, raising both time and monetary costs of mobility.

    \textbf{Ridesourcing} entered this landscape after the city began publishing TNP trip data under open-data regulation. Platforms are often described as filling \textit{``last mile'' gaps} or serving neighbourhoods poorly reached by fixed routes.

    \paragraph{Community Areas} Community Areas (CAs) are the \textbf{city's official statistical neighbourhoods}: 77 contiguous zones used consistently in planning, demographics, and open data. They are coarse for street-level behaviour but appropriate for ecological comparison of \textbf{how ridesourcing, vulnerability, and transit supply co-locate across Chicago}. Five indicators are built at this scale from the open-data sources described in Section~\ref{sec:data-analytics}; formal definitions appear in Section~\ref{sec:indicators}.

\subsection{Smart cities problem selection}
    The compensatory narrative is politically appealing: \emph{if TNP intensity is highest where CTA is weakest and populations are most vulnerable, private mobility could be read as an informal safety net. If instead intensity peaks where transit is already strong and incomes higher, platforms reinforce existing urban hierarchies rather than correcting them.}\\

    This report tests \textbf{which story the spatial data support} at Community Area scale in 2024, focusing on \textbf{ridesourcing equity in Chicago}.

    To do so, we use \textbf{five indicators} at Community Area level: \textbf{TUI} ("Transport Usage Index", a.k.a. ridesourcing usage intensity), \textbf{SDVI/HSVI} (socio-demographic vulnerability), \textbf{Weighted TUI} (income-normalised financial burden), mean \textbf{travel time to the Loop}, and the \textbf{Transit Accessibility Index} (CTA infrastructure and GTFS weekday frequency).
    Their data inputs and formal definitions are presented in Section~\ref{sec:data-analytics} and Section~\ref{sec:indicators} respectively.

    With these indicators, we analyse \textbf{75} of Chicago's 77 CAs in 2024. CA~76 (the airport, O'Hare) is excluded as a transport-hub outlier, and CA~77 (Edgewater) is dropped because 2024 TUI is missing.\\


    The \textbf{scale of inequality} is already visible in TUI: in 2024, central areas such as the Loop, Near North Side, and Near West Side record more than 8{,}000 TNP pickups per 1{,}000 residents, while peripheral East Side (CA~52) and Hegewisch (CA~55) remain below 500.\\
    This \textbf{centre--periphery gap} is the \textbf{empirical starting point for the research question} we chose:
    \begin{highlightbox}
        \textbf{Research question.} Does ridesourcing act as a mobility equaliser in transit-poor, vulnerable peripheries, or as a costly substitute where public transport is already strong?
        \end{highlightbox}


\subsection{Problem importance and urban impact}

    Ridesourcing is now discussed as part of urban mobility policy, rather than a mere private transport service. It is therefore important for many types of people, the so-called \textbf{stakeholders}: starting from \textbf{residents} in high-hardship peripheral areas, where mobility often depends on long CTA trips or expensive TNP rides to reach jobs and services (a \$20-\$30 trip to the Loop weighs much more on household budgets in Riverdale than in Lincoln Park).  Passing then through the \textbf{planners}, that could be tempted to treat TNP as an automatic substitute for public transport investment, not paying attention to the fact that if ridesourcing is already strongest where CTA supply is dense, partnerships for first/last-mile service may duplicate existing coverage instead of filling real gaps.
    Lastly, \textbf{operators and regulators} can be interested in the open trip data, that can support two opposite narratives: mobility equalisation in underserved areas, or market concentration in central zones that are already well connected.

    We therefore test the \textbf{compensatory hypothesis}: \emph{if ridesourcing fills public transport gaps, TUI should be highest in transit-poor and vulnerable peripheries. If not, platforms likely reinforce existing centre--periphery inequalities rather than correcting them.}
...
\begin{table}[htbp]
    ...
    \small
    \begin{tblr}{
        colspec = {Q[4cm,c,l] Q[9.3cm,p,l,leftsep=1mm,rightsep=1mm]},
        row{1} = {font=\bfseries},
        hline{1,Z} = {solid},
        vline{1-Z} = {solid},
    }
        Issue & Implication \\
        Non-resident pickup bias &
        TUI can be inflated in central CAs by non-resident activity because \textbf{TNP records lack passenger home location}. \\
        Temporal data mismatch &
        Vulnerability inputs (hardship and income, 2008--2012 vintage) pre-date 2024 trips. GTFS schedules reflect the 2026 feed while trip counts are from 2024. \\
        Misreading low TUI &
        Low-TUI maps risk labelling peripheral communities as \textbf{``disengaged''} when they may be \textbf{underserved and income-constrained}. Results should pair intensity maps with \textbf{Weighted TUI} and transit deficit. \\
    \end{tblr}
\end{table}

\newpage

\section{Data Analytics, Optimization and Policy Suggestions}\label{sec:data-analytics}

This section documents how open data were assembled, cleaned, and analysed to test the compensatory hypothesis. We proceed from source description through spatial diagnostics, inferential models, and map-based interpretation, then summarise policy implications. 

\subsection{Dataset identification and selection}
All inputs are \textbf{open municipal or regional datasets}, specifically selected thanks to their capacity in covering the \textbf{three dimensions} of the research question: \textbf{ridesourcing demand}, \textbf{socioeconomic context}, and \textbf{public transport supply}.\\
Here below the description of each dataset.

\paragraph{\datasettitle{TNP monthly trip counts (2018--2024)}} Chicago publishes Transportation Network Provider records through \textbf{two successive open-data releases}: one covering \textbf{2018--2022} and a second covering \textbf{2023--2024} \cite{chicago_tnp,chicago_tnp_legacy}. Codebase queries each release through the \textbf{city Socrata API} and asks the server to return \textbf{monthly pickup counts by CA} whereas the record has a valid pickup area. The two extracts are combined into a single monthly series, from which we derive calendar years. These counts form the \textbf{numerator of the Transport Usage Index} and support the descriptive comparison between \textbf{2018 and 2024}.

\paragraph{\datasettitle{TNP median fares by Community Area (2023--2024)}} It provides, for each pickup Community Area and calendar year, the \textbf{median total amount charged per trip}. Original data is divided in individual rides, therefore the codebase asks the portal to \textbf{aggregate on the server} in order to obtain a \textbf{small summary table} (one median fare per zone per year). The \textbf{2024 values} convert annual trip volumes into \textbf{estimated ridesourcing expenditure} in the \textbf{Weighted TUI}, using \textbf{area-specific price levels} instead of a uniform citywide fare. Unfortunately, we didn't find data before 2023-2024 window, so no comparison between periods can be made.

\paragraph{\datasettitle{TNP travel time to the Loop (2024)}}  For each month we obtain the \textbf{mean travel time} and the \textbf{number of trips per pickup area}. Trip duration is also limited to the \textbf{2023--2024 release}. Because a single annual query would be too large, we request data \textbf{one month at a time} for trips \textbf{ending in the Loop}, with a valid pickup Community Area and with \textbf{O'Hare excluded as an origin}. Failed requests are \textbf{retried up to three times}. Monthly results are combined into an \textbf{annual trip-weighted average} per Community Area. This indicator captures \textbf{realised ridesourcing travel time to the central business district}.

\paragraph{\datasettitle{Population (CMAP Community Data Snapshots)}:} Resident population denominators come from the \textbf{Chicago Metropolitan Agency for Planning Community Data Snapshots} \cite{cmap_population}. Annual files provide \textbf{total population by Community Area}. We read all available snapshot years, align neighbourhood names and numeric identifiers across files, and build a \textbf{panel of population by area and year}. Population is then matched to monthly trip counts before the Transport Usage Index is calculated, so the rate reflects \textbf{trips per resident rather than per daytime visitor}.
...
Table~\ref{tab:datasets} condenses \textbf{sources, coverage, spatial unit, and analytical role}. The paragraphs above describe what each dataset contains and how it was obtained or prepared for analysis.
...
\subsection{Indicator construction}\label{sec:indicators}
Once the datasets above are merged and cleaned, we construct five ecological indicators at Community Area level. Each value summarises a neighbourhood rather than an individual rider. Indicators built from z-scored components are comparable across CAs. Table~\ref{tab:indicators} lists their roles; equations below define computation.
